...
- "정사각형 모양으로 배치해줘" → `shape: square` (가로/세로 직사각형도)
...
**LLM 설정은 기본적으로 접혀 있습니다** — 노드에는 지시 입력칸만 보이고,
제공자·주소·모델·토큰·연결/키 버튼은 노드 우클릭 → **Show LLM options**
로 펼칩니다(워크플로우마다 기억). 같은 워크플로우에 같은 지시로 다시
정렬하면(예: 노드를 둔 채 Queue를 반복) **이전 LLM 계획을 재사용**해서
유료 API를 매번 다시 부르지 않습니다.

**진행 표시**: LLM 왕복은 모델에 따라 수십 초가 걸릴 수 있어, 정렬이
도는 동안 **Sort now 버튼이 실시간 상태로 바뀝니다** —
`⏳ Asking the LLM… 12s` → `⏳ Computing layout… 13s` → 완료 시 원상복구.
(LLM 응답의 남은 시간은 원리상 알 수 없어 퍼센트 대신 단계+경과초로
정직하게 표시합니다.)
