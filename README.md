...
- **`grid` (기본)** — 깔끔하게 딱 떨어지는 격자 스타일. 열들이 위쪽
  모서리를 공유하고(그룹 내부는 그룹의 좌상단 기준), 각 노드의 폭을 열
  폭에 맞춰 통일하며(넓히기만 하고 절대 줄이지 않음), 모든 좌표를 10px
  캔버스 그리드에 스냅합니다.
- **`flow`** — 열을 세로 중앙축에 정렬하고 노드 크기를 원본 그대로
  유지합니다. 열 높이 차이가 클 때 대각선 링크가 짧아져 흐름을 따라
  읽기 좋습니다.
