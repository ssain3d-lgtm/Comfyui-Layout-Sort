...
- **그룹 프레임을 선택하고 정렬하면 프레임은 그대로** — 노드가 든 그룹
  ...
  변하지 않고**, 내부 노드들만 프레임의 내부 공간(비율 포함)에 맞춰
  정리됩니다. 내용이 프레임보다 크면 노드 크기를 줄이는 대신 우/하단으로
  넘치고 경고 토스트로 알려줍니다 — 프레임을 키우거나 간격을 줄이세요.
  (중첩된 하위 그룹의 프레임은 내용에 맞게 조정됩니다. 전체 정렬에서는
  내용 배치가 통째로 바뀌므로 프레임도 함께 다시 감싸집니다.)
