...
- **`inner`** — **매크로 배치 유지 모드.** 사람이 잡아둔 그룹의 위치
  ...
  프레임 크기를 내용에 맞춥니다. 그룹 밖 노드는 전혀 건드리지 않습니다.
  큰 그림은 이미 마음에 들고 내부만 지저분할 때 쓰세요. (내용이 커지면
  이웃 그룹과 겹칠 수 있습니다 — 위치를 존중하는 대가입니다.)
...
- **`grid` (기본)** — 열들이 위쪽 모서리를 공유하고(그룹 내부는 그룹의
  좌상단 기준), 모든 좌표를 10px 캔버스 그리드에 스냅합니다.
- **`flow`** — 열을 세로 중앙축에 정렬합니다. 열 높이 차이가 클 때
  대각선 링크가 짧아져 흐름을 따라 읽기 좋습니다.

어느 스타일에서도 **노드 크기는 절대 변경하지 않습니다** — 정렬기는
위치만 옮깁니다.
